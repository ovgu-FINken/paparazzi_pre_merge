...
#include "modules/finken_model/finken_model_landing.h"
...
struct system_model_s finken_landing_model;
struct actuators_model_s finken_actuators_set_point;

void update_actuators_set_point(void);

void finken_landing_model_init(void) {
  finken_landing_model.distance_z     = 0.0;
  finken_landing_model.velocity_theta = 0.0;
  finken_landing_model.velocity_x     = 0.0;
  finken_landing_model.velocity_y     = 0.0;
  ...
  register_periodic_telemetry(DefaultPeriodic, "FINKEN_SYSTEM_MODEL", send_finken_landing_model_telemetry);
}

void finken_landing_model_periodic(void)
{
	update_finken_landing_model();
...
void update_finken_landing_model(void)
{
	if(finken_sensor_model.distance_z < 2.5) {
		finken_landing_model.distance_z     = finken_sensor_model.distance_z;
	}
	
  finken_landing_model.velocity_theta = finken_sensor_model.velocity_theta;
  finken_landing_model.velocity_x     = finken_sensor_model.velocity_x;
  finken_landing_model.velocity_y     = finken_sensor_model.velocity_y;
}

/*
 * Use finken_landing_set_point to calculate new actuator settings
 */
float sum_error_x = 0;
float sum_error_y = 0;
float sum_error_z = 0;
...
void send_finken_landing_model_telemetry(struct transport_tx *trans, struct link_device* link)
{
  trans=trans;
  link=link;
  DOWNLINK_SEND_FINKEN_SYSTEM_MODEL(
    DefaultChannel,
    DefaultDevice,
    &finken_landing_model.distance_z,
    &finken_landing_model.velocity_theta,
    &finken_landing_model.velocity_x,
    &finken_landing_model.velocity_y,
