...
#include "modules/finken_model/finken_model_starting.h"
...
struct system_model_s finken_starting_model;
struct actuators_model_s finken_actuators_set_point;

void update_actuators_set_point(void);

void finken_starting_model_init(void) {
  finken_starting_model.distance_z     = 0.0;
  finken_starting_model.velocity_theta = 0.0;
  finken_starting_model.velocity_x     = 0.0;
  finken_starting_model.velocity_y     = 0.0;
  ...
  register_periodic_telemetry(DefaultPeriodic, "FINKEN_SYSTEM_MODEL", send_finken_starting_model_telemetry);
}

void finken_starting_model_periodic(void)
{
	update_finken_starting_model();
	update_actuators_set_point();
}

void update_finken_starting_model(void)
{
	if(finken_sensor_model.distance_z < 2.5) {
		finken_starting_model.distance_z     = finken_sensor_model.distance_z;
	}
	
  finken_starting_model.velocity_theta = finken_sensor_model.velocity_theta;
  finken_starting_model.velocity_x     = finken_sensor_model.velocity_x;
  finken_starting_model.velocity_y     = finken_sensor_model.velocity_y;
}

/*
 * Use finken_starting_set_point to calculate new actuator settings
 */
float sum_error_x = 0;
float sum_error_y = 0;
float sum_error_z = 0;
...
void update_actuators_set_point()
{
    ...
    //float error_z = finken_landing_set_point.distance_z - finken_landing_model.distance_z;
    ...
    //float velocity_z = (finken_landing_model.distance_z - distance_z_old) * FINKEN_SYSTEM_UPDATE_FREQ;
    ...
    //distance_z_old = finken_landing_model.distance_z;
    
    // TODO: Theta
}

void send_finken_starting_model_telemetry(struct transport_tx *trans, struct link_device* link)
{
  trans=trans;
  link=link;
  DOWNLINK_SEND_FINKEN_SYSTEM_MODEL(
    DefaultChannel,
    DefaultDevice,
    &finken_starting_model.distance_z,
    &finken_starting_model.velocity_theta,
    &finken_starting_model.velocity_x,
    &finken_starting_model.velocity_y,
