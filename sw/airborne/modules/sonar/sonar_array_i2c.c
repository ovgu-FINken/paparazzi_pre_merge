...
static const char read_order[] = { SONAR_ADDR_FRONT, SONAR_ADDR_RIGHT,
SONAR_ADDR_BACK, SONAR_ADDR_LEFT };
...
struct i2c_transaction sonar_i2c_write_trans[4];
 ...
 */
static void sonar_send_command(uint8_t index) {
	uint8_t i2c_addr = read_order[index];
	sonar_i2c_write_trans[index].buf[0] = 0x51;
	i2c_transmit(&SONAR_I2C_DEV, &sonar_i2c_write_trans[index],
			(i2c_addr << 1) & ~0x01, 1); // 7-Bit Adress + write Bit (last bit set to 0)
}

static void query_sensor(uint16_t* value, uint16_t* old_value, uint8_t i2c_addr,
		struct i2c_transaction* transaction) {
	if (transaction->status != I2CTransPending
			&& transaction->status != I2CTransRunning) {
		i2c_receive(&SONAR_I2C_DEV, transaction, (i2c_addr << 1) | 1, 2);
		int16_t meas = (int16_t) (((uint16_t) (transaction->buf[0]) << 8)
				| (uint16_t) (transaction->buf[1]));	// recieve mesuarment
		if (meas > 0) {
			*old_value = *value;
			*value = meas;
		}
...
#ifndef SITL
	query_sensor(&sonar_values.front, &sonar_values_old.front, SONAR_ADDR_FRONT,
			&sonar_i2c_read_front_trans);
	query_sensor(&sonar_values.right, &sonar_values_old.right, SONAR_ADDR_RIGHT,
			&sonar_i2c_read_right_trans);
	query_sensor(&sonar_values.back, &sonar_values_old.back, SONAR_ADDR_BACK,
			&sonar_i2c_read_back_trans);
	query_sensor(&sonar_values.left, &sonar_values_old.left, SONAR_ADDR_LEFT,
			&sonar_i2c_read_left_trans);
#endif
}
...
void sonar_array_i2c_periodic(void) {
	...
	// single
	//sonar_send_command(sonar_index);

	// pairwise
	sonar_send_command(sonar_index % 2);
	sonar_send_command((sonar_index % 2) + 2);

	// all-out
//	sonar_send_command(0);
//	sonar_send_command(1);
//	sonar_send_command(2);
//	sonar_send_command(3);
...
void send_sonar_array_telemetry(struct transport_tx *trans,
	...
	DOWNLINK_SEND_SONAR_ARRAY(DefaultChannel, DefaultDevice,
			...
			&sonar_values.left, &sonar_fail_telemetry_pitch,
